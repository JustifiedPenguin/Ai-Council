...
MODERATOR_SYSTEM = "You are a neutral moderator summarizing a council debate. Synthesize key points of agreement and disagreement, then give a clear actionable recommendation. Be concise. End with a single clear recommendation."
    ...
    try:
        ...
        if b == "DuckDuckGo":
            ...
            if data.get("AbstractText"):
                ...
                if data.get("AbstractURL"): sources.append(data["AbstractURL"])
            ...
            if b == "Brave Search":
                ...
                sources = [x.get("url", "") for x in items if x.get("url")]
                return "\n\n".join(f"{x['title']}: {x.get('description','')}" for x in items), sources
            else:
                r     = requests.post("https://api.tavily.com/search",
                    json={"api_key": key, "query": query, "max_results": n}, timeout=5)
                items = r.json().get("results", [])[:n]
                sources = [x.get("url", "") for x in items if x.get("url")]
                return "\n\n".join(f"{x['title']}: {x.get('content','')}" for x in items), sources
...
def get_context_size(backend, model_name=""):
    ...
    try:
        if backend in ("llama.cpp", "Odysseus"):
            ...
            r   = requests.post(f"{LLAMA_HOST}/api/show", json={"name": model_name}, timeout=2)
            ctx = r.json().get("model_info", {}).get("llama.context_length")
            if ctx: return ctx
        elif backend == "koboldcpp":
            r = requests.get(f"{LLAMA_HOST}/api/extra/true_max_context_length", timeout=2)
            n = r.json().get("value")
            if n: return n
    ...
    def ask(self, system, messages):
        try:
            r = requests.post(LLAMA, json={
                ...
                "max_tokens": min(settings.get("max_tokens", 512), self.ctx_size // 16),
            }, timeout=30)
    ...
    def run(self):
        ...
        if settings["search_enabled"]:
            ...
            if results:
                ...
                self.search_done.emit(f"SOURCES:\n{sources_text}\n\nCONTEXT:\n{results}")
            ...
            for p in personas:
                ...
                if round_num > 0:
                    others = "\n\n".join(
                        ...
                        for name, text in responses.items() if name != p["name"]
                    )
                    histories[p["name"]].append({"role": "user",
        ...
        summary = self.ask(MODERATOR_SYSTEM,
            [{"role": "user", "content": f"The council debated: '{self.question}'\n\n{all_responses}"}])
...
class PersonaCard(QFrame):
    def __init__(self, name, color):
        ...
        self.collapse_btn.setStyleSheet(
            f"QPushButton{{background:transparent;border:none;color:{color}88;font-size:14px;}}"
    ...
    def _bg(self): return settings.get("card_bg", "#1a1a1a")
...
class RoundIndicator(QWidget):
    ...
    def _pill_style(self, state):
        ...
        if state == "done":
            return (f"QLabel{{background:#1a1a1a;border:1px solid {a}44;color:{a}66;"
                    f"font-size:9px;letter-spacing:1px;padding:2px 10px;border-radius:11px;}}")
        if state == "active":
    ...
    def reset(self):
        for pill in self.pills: pill.setStyleSheet(self._pill_style("idle"))
...
class PersonaEditorDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            "QDialog{background:#111;} QWidget{background:#111;color:#ccc;} QLabel{background:transparent;}"
        ...
        title.setStyleSheet(
            f"color:{settings['accent']};font-size:11px;letter-spacing:3px;font-weight:bold;")
...
class SettingsPanel(QFrame):
    ...
    def __init__(self, parent):
        ...
        self.setStyleSheet("""
            ...
            QSpinBox {
                background: #1a1a1a; border: 1px solid #444; color: #C8A96E; padding: 4px 8px;
            }
        ...
        self.accent_btn.setStyleSheet(
            f"background:{settings['accent']};border:1px solid #555;border-radius:4px;")
        ...
        self.card_bg_btn.setStyleSheet(
            f"background:{settings['card_bg']};border:1px solid #555;border-radius:4px;")
        ...
        self.bg_btn.setStyleSheet(
            f"background:{settings.get('bg_color','#0f0f0f')};border:1px solid #555;border-radius:4px;")
        ...
        self.notif_check = QCheckBox("KDE notifications")
        ...
        self.backend_combo.addItems([
            ...
            "Jan","koboldcpp","Oobabooga","TabbyAPI","OpenAI / OpenRouter"
        ])
        ...
        self.search_backend_combo.currentTextChanged.connect(self._on_search_backend)
        lrow("Search engine", self.search_backend_combo)

        self.search_url_lbl = QLabel("SearXNG URL")
        self.search_url_lbl.setFixedWidth(110)
        self.search_url_lbl.setStyleSheet("color:#888;font-size:11px;background:transparent;")
        ...
        self.search_key_lbl.setStyleSheet("color:#888;font-size:11px;background:transparent;")
        ...
        self.search_key_input.textChanged.connect(lambda v: self._set("search_api_key", v))
        sk_r = QHBoxLayout()
        sk_r.setSpacing(8)
    ...
    def _reset_color(self, key, default, btn):
        ...
        btn.setStyleSheet(f"background:{default};border:1px solid #555;border-radius:2px;")
    ...
    def test_connection(self):
        ...
        try:
            ...
            self.test_result.setText(f"✓ Connected — ctx: {ctx} — replied: {reply[:40]}")
        except Exception as e:
    ...
    def test_search(self):
        ...
        try:
            ...
            if results and not results.startswith("["):
                ...
                self.search_test_result.setText(f"✓ Search working — {len(sources)} source(s) found")
            else:
                self.search_test_result.setStyleSheet("color:#e67e22;font-size:10px;background:transparent;")
                self.search_test_result.setText("⚠ Connected but no results returned")
        except Exception as e:
    ...
    def _reset_all(self):
        ...
        self.accent_btn.setStyleSheet(
            f"background:{DEFAULT_SETTINGS['accent']};border:1px solid #555;border-radius:2px;")
        self.card_bg_btn.setStyleSheet(
            f"background:{DEFAULT_SETTINGS['card_bg']};border:1px solid #555;border-radius:2px;")
        ...
        self.search_backend_combo.setCurrentText(DEFAULT_SETTINGS["search_backend"])
...
class CouncilApp(QMainWindow):
    def __init__(self):
        ...
        a = settings["accent"]
        hbtn = (f"QPushButton{{background:transparent;border:1px solid #2a2a2a;color:#666;"
                f"font-size:10px;letter-spacing:1px;padding:4px 12px;border-radius:3px;}}"
        ...
        self.settings_btn.setStyleSheet(
            f"QPushButton{{background:transparent;border:1px solid #2a2a2a;color:#666;"
            f"font-size:16px;border-radius:3px;}}"
            f"QPushButton:hover{{border-color:{a}77;color:{a};}}"
    ...
    def _pulse_btn(self):
        ...
        a = settings["accent"]
        self.status_label.setStyleSheet(f"color:{a};font-size:11px;letter-spacing:1px;font-weight:bold;")
        self.status_label.setText(f"{frames[self._btn_pulse_state]}  deliberating...")
    ...
    def _on_settings_changed(self):
        ...
        self.setStyleSheet(f"QMainWindow{{background:{bg};}} QWidget{{background:{bg};color:#cccccc;}} QLabel{{background:transparent;}}")
        self._header_frame.setStyleSheet(
            f"QFrame{{background:qlineargradient(x1:0,y1:0,x2:0,y2:1,"
    ...
    def export_debate(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export Debate", "council_debate.md", "Markdown (*.md);;Text (*.txt)")
        if not path: return
    ...
    def on_verdict(self, text):
        ...
        if settings["notifications"]:
            try:
                subprocess.run(["notify-send", "-h",
                    "string:sound-name:message-new-instant",
                    "The Council has spoken", "Your verdict is ready."])
            except: pass
